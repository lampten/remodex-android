...
const CODEX_HANDSHAKE_STATE_COLD = "cold";
const CODEX_HANDSHAKE_STATE_INITIALIZE_ACKED = "initialize_acked";
const CODEX_HANDSHAKE_STATE_WARM = "warm";
...
function startBridge({
  ...
  let codexHandshakeState = config.codexEndpoint
    ? CODEX_HANDSHAKE_STATE_WARM
    : CODEX_HANDSHAKE_STATE_COLD;
  ...
  function handleBridgeManagedHandshakeMessage(rawMessage) {
    ...
    if (method === "initialize" && parsed.id != null) {
      if (!shouldShortCircuitBridgeInitialize(codexHandshakeState)) {
    ...
    if (method === "initialized") {
      const decision = consumeBridgeInitializedNotification(codexHandshakeState);
      codexHandshakeState = decision.nextState;
  ...
  function trackCodexHandshakeState(rawMessage) {
    ...
    codexHandshakeState = nextCodexHandshakeStateAfterInitializeResponse(
      codexHandshakeState,
      parsed
    );
  }
...
// Registers the canonical Mac identity and all trusted iPhones allowed for auto-resolve.
function buildMacRegistrationHeaders(deviceState) {
  ...
  if (registration.trustedPhones.length > 0) {
    headers["x-trusted-phones-json"] = JSON.stringify(registration.trustedPhones);
  }
...
function buildMacRegistration(deviceState) {
  ...
  const trustedPhoneEntry = trustedPhones[0] || null;
  return {
    ...
    trustedPhoneDeviceId: normalizeNonEmptyString(trustedPhoneEntry?.deviceId),
    trustedPhonePublicKey: normalizeNonEmptyString(trustedPhoneEntry?.publicKey),
    trustedPhones,
  };
}
...
function shouldShortCircuitBridgeInitialize(handshakeState) {
  return handshakeState === CODEX_HANDSHAKE_STATE_WARM;
}

function consumeBridgeInitializedNotification(handshakeState) {
  if (handshakeState === CODEX_HANDSHAKE_STATE_WARM) {
    return {
      intercept: true,
      nextState: CODEX_HANDSHAKE_STATE_WARM,
    };
  }

  if (handshakeState === CODEX_HANDSHAKE_STATE_INITIALIZE_ACKED) {
    return {
      intercept: false,
      nextState: CODEX_HANDSHAKE_STATE_WARM,
    };
...
function nextCodexHandshakeStateAfterInitializeResponse(handshakeState, parsedResponse) {
  if (parsedResponse?.result != null) {
    return CODEX_HANDSHAKE_STATE_INITIALIZE_ACKED;
  }

  const errorMessage = typeof parsedResponse?.error?.message === "string"
    ? parsedResponse.error.message.toLowerCase()
    : "";
  if (errorMessage.includes("already initialized")) {
    return CODEX_HANDSHAKE_STATE_WARM;
  }
...
module.exports = {
  ...
  consumeBridgeInitializedNotification,
  hasRelayConnectionGoneStale,
  nextCodexHandshakeStateAfterInitializeResponse,
  sanitizeThreadHistoryImagesForRelay,
  shouldShortCircuitBridgeInitialize,
  startBridge,
};
