...
test("bridge only short-circuits initialize after the Codex handshake is fully warm", () => {
  assert.equal(shouldShortCircuitBridgeInitialize("cold"), false);
  assert.equal(shouldShortCircuitBridgeInitialize("initialize_acked"), false);
  assert.equal(shouldShortCircuitBridgeInitialize("warm"), true);
});

test("bridge forwards initialized once after the first initialize response, then short-circuits reconnects", () => {
  assert.deepEqual(
    consumeBridgeInitializedNotification("initialize_acked"),
    {
      intercept: false,
      nextState: "warm",
    }
  ...
  assert.deepEqual(
    consumeBridgeInitializedNotification("warm"),
    {
      intercept: true,
      nextState: "warm",
    }
...
test("initialize responses move the handshake from cold to awaiting initialized", () => {
  assert.equal(
    nextCodexHandshakeStateAfterInitializeResponse("cold", {
      id: "initialize-1",
      result: {
        serverInfo: {
          name: "codex",
...
test("already initialized responses keep the handshake warm for reconnects", () => {
  assert.equal(
    nextCodexHandshakeStateAfterInitializeResponse("cold", {
      id: "initialize-2",
      error: {
