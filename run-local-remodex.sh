...
const localNodeModulesDir = path.join(packageDir, "node_modules");

for (const dependencyName of dependencyNames) {
  try {
    const resolvedPath = requireFromPackage.resolve(`${dependencyName}/package.json`);
    const relativePath = path.relative(localNodeModulesDir, resolvedPath);
    if (
      !relativePath
