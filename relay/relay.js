...
function resolveTrustedMacSession({
  ...
  if (!findTrustedPhoneRegistration(
    liveSession,
    normalizedPhoneDeviceId,
    normalizedPhoneIdentityPublicKey
  )) {
...
function readMacRegistrationHeaders(headers, sessionId) {
  return normalizeMacRegistration({
    ...
    trustedPhones: readTrustedPhonesHeader(headers["x-trusted-phones-json"]),
...
function normalizeMacRegistration(registration, sessionId) {
  const trustedPhones = normalizeTrustedPhones(
    ...
    {
      deviceId: normalizeNonEmptyString(registration?.trustedPhoneDeviceId),
      publicKey: normalizeNonEmptyString(registration?.trustedPhonePublicKey),
    }
  );
...
function findTrustedPhoneRegistration(macRegistration, phoneDeviceId, phoneIdentityPublicKey) {
  ...
  const trustedPhones = Array.isArray(macRegistration?.trustedPhones)
    ? macRegistration.trustedPhones
    : [];
  for (const trustedPhone of trustedPhones) {
    if (
      trustedPhone?.deviceId === normalizedDeviceId
      && trustedPhone?.publicKey === normalizedPublicKey
    ) {
      return trustedPhone;
    }
  }
  ...
  if (
    macRegistration?.trustedPhoneDeviceId === normalizedDeviceId
    && macRegistration?.trustedPhonePublicKey === normalizedPublicKey
  ) {
    return {
      deviceId: macRegistration.trustedPhoneDeviceId,
      publicKey: macRegistration.trustedPhonePublicKey,
    };
  }
...
function readTrustedPhonesHeader(headerValue) {
  const normalized = readHeaderString(headerValue);
  if (!normalized) {
    return [];
  }
...
function normalizeTrustedPhones(rawTrustedPhones, fallbackTrustedPhone = null) {
  const normalizedTrustedPhones = [];
  const seenDeviceIds = new Set();

  const appendTrustedPhone = (deviceId, publicKey) => {
    const normalizedDeviceId = normalizeNonEmptyString(deviceId);
    const normalizedPublicKey = normalizeNonEmptyString(publicKey);
    if (!normalizedDeviceId || !normalizedPublicKey || seenDeviceIds.has(normalizedDeviceId)) {
      return;
    }

    seenDeviceIds.add(normalizedDeviceId);
    normalizedTrustedPhones.push({
      deviceId: normalizedDeviceId,
      publicKey: normalizedPublicKey,
    });
  };

  if (Array.isArray(rawTrustedPhones)) {
    for (const trustedPhone of rawTrustedPhones) {
      appendTrustedPhone(trustedPhone?.deviceId, trustedPhone?.publicKey);
    }
  } else if (rawTrustedPhones && typeof rawTrustedPhones === "object") {
    for (const [deviceId, publicKey] of Object.entries(rawTrustedPhones)) {
      appendTrustedPhone(deviceId, publicKey);
    }
  }

  appendTrustedPhone(fallbackTrustedPhone?.deviceId, fallbackTrustedPhone?.publicKey);
  return normalizedTrustedPhones;
}
